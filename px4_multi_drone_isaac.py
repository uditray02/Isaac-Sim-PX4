...
class PegasusApp:
    ...
    def vehicle_factory(self, vehicle_id: int, gap_x_axis: float):
        ...
        backend = PX4MavlinkBackend(mavlink_config)
        backend.enable_state_machine = True
        backend.enable_autonomy = True
        backend.enable_control = True

        config_multirotor.backends = [backend]


        Multirotor(
            f"/World/quadrotor_{vehicle_id}",
            ROBOTS['Iris'],
            vehicle_id,
